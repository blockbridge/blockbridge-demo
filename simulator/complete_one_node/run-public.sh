#!/bin/bash
###############################################################################
# Run a Blockbridge simulator management node and 1 storage node with IP
...
: ${BB_MN_SIM_IP:?"BB_MN_SIM_IP (management node IP address) is not set and could not be detected"}
...
: ${BB_SN_SIM_IP:?"BB_SN_SIM_IP (storage node IP address) is not set and could not be detected"}
...
echo -n "Starting management node (using $BB_MN_SIM_IP): "
...
echo -n "Starting storage node (using $BB_SN_SIM_IP): "
